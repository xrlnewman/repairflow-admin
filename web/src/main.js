...
function mobileView() {
  return `<section class="mobile-panel"><div class="mobile-panel__hero"><span>REPAIRFLOW MOBILE</span><h2>我的工单与服务跟进</h2><p>移动端与工单 API 共享报价、派工、上门、验收和质保记录。</p><button class="primary" data-action="create-appointment">＋ 创建演示工单</button></div><div class="mobile-list"><h3>今日工单</h3>${appointments.slice(0, 4).map((appointment) => `<article class="mobile-card"><div><small>${timeLabel(appointment.scheduledAt)} · ${appointment.department}</small><strong>${appointment.patient}</strong><span>${appointment.doctor} · ${appointment.status}</span></div><b class="status ${statusColors[appointment.status] || 'indigo'}">${appointment.status}</b>${appointmentAction(appointment)}</article>`).join('')}</div><div class="mobile-list"><h3>我的服务跟进</h3>${followupTasks.slice(0, 3).map((item) => `<article class="mobile-card"><div><small>${item.dueAt}</small><strong>${item.summary}</strong><span>${item.patient} · ${item.status}</span></div>${item.status === '已完成' ? '<b class="status green">已完成</b>' : `<button class="text-action" data-action="complete-followup" data-followup-id="${item.id}">完成跟进</button>`}</article>`).join('')}</div></section>`
}
